...
def load_meters():
    df = session.query("""
        SELECT *
        FROM DASHBOARDS_DB.WATER.METERS
...
def load_basement():
    df = session.query("""
        SELECT *
        FROM DASHBOARDS_DB.WATER.BASEMENT
        WHERE DATE >= CURRENT_DATE() - INTERVAL '1 YEAR'
        ORDER BY DATE DESC, TIME DESC
    """)
    df = df.rename(columns={col: col.title() for col in df.columns})
...
def load_totals():
    df = session.query("""
        SELECT *
        FROM DASHBOARDS_DB.WATER.TOTALS
        WHERE DATE >= CURRENT_DATE() - INTERVAL '1 YEAR'
        ORDER BY DATE DESC
    """)
    df = df.rename(columns={col: col.title() for col in df.columns})
...
def load_data():
    meters = load_meters()
    basement = load_basement()
    totals = load_totals()
    df = totals.merge(basement, on=["Year", "Month", "Day", "Date", "Time", "Week"], how="outer")
    df = df.merge(meters, on=["Year", "Month", "Day", "Date", "Time", "Week"], how="outer")
...
def main():
    ...
    if not filtered_data.empty:
        ...
        # Use filtered_data as the time series dataframe
        if len(filtered_data) > 5000:
            # Sample every nth point to reduce rendering load
            sample_rate = max(1, len(filtered_data) // 3000)
            filtered_ts_display = filtered_data.iloc[::sample_rate].copy()
            st.info(f"Displaying every {sample_rate} data points for better performance ({len(filtered_ts_display)} of {len(filtered_data)} points)")
        else:
            filtered_ts_display = filtered_data
            
        for meter in water_meter_columns:
            if meter in filtered_ts_display.columns:
                fig.add_trace(go.Scatter(
                    x=filtered_ts_display[x_col],
                    y=filtered_ts_display[meter],
                    mode='lines+markers',
    ...
    if not filtered_data.empty and "Basement Main" in filtered_data.columns and "CONSUMPTION (L)" in filtered_data.columns:
        ...
        fig_incoming_line.add_trace(go.Scatter(
            x=filtered_ts_display[x_col],
            y=filtered_ts_display["Basement Main"],
            mode='lines+markers',
        ...
        fig_incoming_line.add_trace(go.Scatter(
            x=filtered_ts_display[x_col],
            y=filtered_ts_display["CONSUMPTION (L)"],
            mode='lines+markers',
        ...
        if meter_cols:
            ...
            fig_incoming_line.add_trace(go.Scatter(
                x=filtered_ts_display[x_col],
                y=filtered_ts_display["Total Meters Sum"],
                mode='lines+markers',
